...
Amber updateNearestPlace(const Amber& amber)
{
    ...
    if (nearestDistance != std::numeric_limits<int>::max())
    {
        ...
        if (abs(nearestDistance) < 1)
        {
            ...
            newAmber.log.push_back(event_log::playerArrivedToPlace(newAmber.nearestPlace));
        }
        else
        {
            newAmber.log.push_back(event_log::playerArrivedToArea(newAmber.nearestPlace));
        }
    }
...
MaybeAmber affectPlayerCurrentShadow(const Amber& amber, const TimedShadowVariator& variator)
{
    ...
    newAmber.log.push_back(event_log::playerAffectedByStormAt(amber.nearestPlace.shadow));
    return monad::maybe::just(newAmber);
}
